...
An RVA (*Relative Virtual Address*) is an offset from a modules image base, it stays the same when ASLR relocates the image, while the absolute virtual address changes.
...
This is useful whenever you want to read a global variable current state that isn't available by name in WinDbg. See [MMCSS values RVA calculation](https://noverse.dev/docs/win-config/system/mmcss-values/#driverstart--rvas) example as a practical use case.
...
It can also be read from the drivers `_DRIVER_OBJECT`:
...
### [_DRIVER_OBJECT](https://learn.microsoft.com/en-us/windows-hardware/drivers/ddi/wdm/ns-wdm-_driver_object) Structure
...
Read the address using the command that matches the variables size. For the `CiSystemResponsiveness` example it is `dd`, so it is a 4 byte DWORD.
...
| IDA | Size | WinDbg |
| --- | --- | --- |
| `db` | 1 byte | `db <address> L1` |
| `dw` | 2 bytes | `dw <address> L1` |
| `dd` | 4 bytes | `dd <address> L1` |
| `dq` | 8 bytes | `dq <address> L1` |

You can practically also display 4 bytes using `db L4`.